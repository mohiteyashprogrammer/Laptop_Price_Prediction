...
class DataTransformation:
    ...
    def get_data_transformation_object(self):
        try:
            ...
            catigorical_features =['Company', 'TypeName', 'ScreenResolution', 'OpSys', 'Cpu_brand','Gpu_brand']

            numerical_features = ['Inches', 'Ram', 'Weight', 'Touchscreen', 'IPS_panel', 'HDD', 'SSD']
    ...
    def initatie_data_transformation(self,train_path,test_path):
        try:
            ...
            targer_feature_train_data = np.log(train_data[target_columns_name])
            

            ## spliting dependent and indipend veriable
            input_features_test_data = test_data.drop(drop_columns,axis=1)
            targer_feature_test_data = np.log(test_data[target_columns_name])
